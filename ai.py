...
def _montar_prompt_fallback(texto: str) -> str:
    ...
    return (
        ...
        "'consulta_dados', 'consulta_contas', 'marcar_contas_pagas', "
        "'indefinido']\n"
        ...
        "de gastos numa categoria — pergunta sobre OS DADOS dele, não sobre "
        "como usar o bot. Cobre dois tipos (campo 'tipo_consulta' abaixo): "
        "'ultimo' (quando foi a última vez, ex: 'qual foi o último dia que "
        "abasteci o carro?') e 'soma' (quanto gastou num período, ex: "
        "'quanto gastei de combustível nos últimos 30 dias?', 'quanto gastei "
        "com mercado esse mês?'). Se a pergunta pedir outra coisa sobre os "
        "dados que não seja essas duas (comparação entre categorias, média, "
        "previsão), use 'indefinido' em vez de forçar 'consulta_dados'.\n"
        "- tipo_consulta: SÓ quando intencao='consulta_dados' — 'ultimo' ou "
        "'soma' (ver acima).\n"
        "- dias: SÓ quando intencao='consulta_dados' e tipo_consulta='soma' "
        "— quantos dias pra trás a pergunta cobre, como NÚMERO (ex: 'esse "
        "mês' -> 30, 'essa semana' -> 7, 'últimos 30 dias' -> 30). Se o "
        "período não ficar claro, use 30 como padrão em vez de null.\n"
        ...
        "  'marcar_contas_pagas' quando o usuário está AVISANDO que pagou "
        "algo que está em aberto nas contas do mês — uma conta específica "
        "OU todas de uma vez (ex: 'paguei a fatura do cartão', 'já paguei "
        "o consórcio', 'quitei a conta de luz', 'todas as contas já foram "
        "pagas', 'já paguei tudo esse mês', 'tá tudo quitado', 'paguei "
        "tudo, pode fechar o mês'). NUNCA use pra registrar um gasto novo "
        "(isso é 'gasto'). Se a frase é uma PERGUNTA em vez de uma "
        "afirmação de que já pagou ('quais contas faltam?', 'já pagaram "
        "tudo?') use 'consulta_contas' em vez disso.\n"
        "- todas: SÓ quando intencao='marcar_contas_pagas' — true se o "
        "usuário quis dizer TODAS as contas em aberto (sem apontar uma "
        "específica), false se apontou uma conta em particular.\n"
        "- descricao_conta: SÓ quando intencao='marcar_contas_pagas' e "
        "todas=false — texto curto descrevendo QUAL conta (ex: 'fatura do "
        "cartão', 'consórcio'), do jeito que a pessoa descreveu, sem "
        "inventar um nome que não está na mensagem.\n"
