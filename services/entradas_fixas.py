...
def lancar_entradas_fixas_do_mes(hoje: date = None) -> list[dict]:
    """
    Lança como entrada normal toda entrada fixa ativa cujo dia efetivo é
    hoje e que ainda não foi lançada neste mês. Idempotente: checagem
    prévia + índice único uq_entrada_fixa_mes contra corrida (mesma dupla
    proteção de lancar_despesas_fixas_do_mes).
    """
